...
class _ProspectState extends State<ListeProspect> {
  ...
  List<String> listeTypesStatut = ['Atente', "Valider", "Rejeter"];
  String typeStatutSelectionne = 'Atente';
  ...
  Widget build(BuildContext context) {
    ...
    var listProspect = context
        .watch<ProspectController>()
        .data;
    return SafeArea(
       child: Scaffold(
      appBar: AppBar(
          ...
          selectionTypeStatut(context),
           Expanded(
              child: (typeStatutSelectionne == "Valider")
                ? listStatutValider(context, context.watch<ProspectController>().listValider)
                  :(typeStatutSelectionne == "Rejeter")
                  ? listStatutRejeter(context,context.watch<ProspectController>().listRejeter)
                  : listStatutAtente(context,context.watch<ProspectController>().listAtente)),
        ],
      ),
    )
    );
  ...
  listStatutValider(BuildContext context, List listValider) {
    return ListView.builder(
      ...
      itemBuilder: (BuildContext context, int index) {
        return ListTile(
          leading: Icon(
            Icons.remove_circle,color: Colors.green,
            size: 35,
          ),
          title: Text(
          ...
          subtitle: Row(
            children: [
              ...
              SizedBox(width: 20),
              InkWell(onTap: null,    child: Icon(Icons.mode))
            ],
          ),
        );
      },
  ...
  listStatutAtente(BuildContext context, List listAtente) {
    return ListView.builder(
      ...
      itemBuilder: (BuildContext context, int index) {
        return ListTile(
          leading: Icon(
            Icons.remove_circle, color: Colors.grey,
            size: 35,
          ),
          title: Text(
        ...
        itemBuilder: (context, index) {
          ...
          return Container(
            ...
            child: ListTile(
              ...
              subtitle: Text(
                'statut :${prospect.state}       |       zone: ${prospect
                    .zone}',
                style: TextStyle(fontSize: 15, color: Colors.black87),
              ),
              trailing: IconButton(
  ...
  selectionTypeStatut(BuildContext context) {
    return Container(
      ...
      child: DropdownButton(
        ...
        items: listeTypesStatut.map((String items) {
